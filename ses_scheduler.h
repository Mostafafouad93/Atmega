...
/*
 * The enum allows more flexible use for task one.
 * Using enume allow the user to choose to toggle
 * which led (RED,GREEN or the YELLOW).
 */
typedef enum ToggleLed {
	RED, GREEN, YELLOW
} Led;
...
struct time_t adjust_time(struct time_t currentTime);
